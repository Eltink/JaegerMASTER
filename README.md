...
Pump actions start on key down and stop on key release. Repeated key events while a key is held are ignored.
...
The current Pi deployment uses `/etc/systemd/system/shotdispenser.service`. While temporary active-high relays are hand-wired, the service includes `--pump-active-high`; remove that flag when switching to the active-low relay HAT.
...
ExecStart=/usr/bin/python3 /home/pi/JaegerMASTER-live-test/main.py --numpad-name Logitech --pump-active-high
Restart=always
RestartSec=3
User=pi
Group=pi
